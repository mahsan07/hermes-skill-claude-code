...
This package expresses **Claude Code** as a typed, task-adaptive agent harness. Its concrete capability remains: Delegate bounded coding tasks to Claude Code CLI with explicit scope and review gates. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Claude Code |
| --- | --- | --- |
| Memory | `before_after_evidence` | Retain bounded coding request plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `prepare_execute_verify_plan` | Transform repository instructions into the ordered, bounded stages defined by this skill. |
...
| Action | `inspect_change_test_handoff` | Advance the workflow through claude code session while preserving stop conditions and user authority. |
